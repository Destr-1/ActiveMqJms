...
@Service
class EmailServiceSender(@Qualifier("eventRepository") override val repository: EventRepository) : Sender {
    override fun send(message: Event) {
        println("email message. Body: ${message.body}, type: ${message.type}")
        val id = message.id
